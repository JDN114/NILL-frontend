...
function mkHullTex() {
  ...
  tex.wrapS = tex.wrapT = THREE.RepeatWrapping; tex.anisotropy = 8; return tex
}
...
function mkSolarTex() {
  ...
  for (let x = 0; x < w; x += cw) {
    for (let y = 0; y < h; y += ch) {
      ...
      s.addColorStop(0, 'rgba(120,180,255,.18)'); s.addColorStop(.5, 'rgba(255,255,255,.05)'); s.addColorStop(1, 'rgba(20,40,90,.2)')
      g.fillStyle = s; g.fillRect(x + 1, y + 1, cw - 2, ch - 2)
      g.fillStyle = 'rgba(180,190,210,.4)'; g.fillRect(x + cw / 2 - .5, y + 1, 1, ch - 2)
    }
  }
...
function mkHaloTex() {
  ...
  grd.addColorStop(0, 'rgba(255,255,255,1)'); grd.addColorStop(.3, 'rgba(255,255,255,.5)'); grd.addColorStop(1, 'rgba(255,255,255,0)')
...
function mkWindowTex() {
  ...
  for (let i = 0; i < 24; i++) {
    ...
    grd.addColorStop(0, 'rgba(255,240,200,1)'); grd.addColorStop(.4, 'rgba(180,220,255,.85)'); grd.addColorStop(1, 'rgba(40,80,160,0)')
...
function buildISSIntoGroup(rootGroup) {
  ...
  const hullMat = new T.MeshStandardMaterial({ map: hullTex, color: 0xffffff, metalness: .65, roughness: .45 })
  const hullMatDark = new T.MeshStandardMaterial({ map: hullTex, color: 0x999fb0, metalness: .7, roughness: .5 })
  const accentMat = new T.MeshStandardMaterial({ color: 0xc6ff3c, emissive: 0x6d8a14, emissiveIntensity: .85, metalness: .5, roughness: .35 })
  ...
  const panelMat = new T.MeshStandardMaterial({ map: solarTex, metalness: .7, roughness: .35, emissive: 0x0a1838, emissiveIntensity: .15, side: T.DoubleSide })
  const winMat = new T.MeshBasicMaterial({ map: winTex, transparent: true, opacity: .95 })

  /* Hub: 3 cylindrical segments + flanges + window strips */
  ...
  const cupolaDome = new T.Mesh(
    ...
    new T.MeshStandardMaterial({ color: 0x4a78b8, metalness: .9, roughness: .1, emissive: 0x3a5878, emissiveIntensity: .4, transparent: true, opacity: .85 })
  )
  ...
  ;[-2.55, 2.55].forEach(x => {
    ...
    cap.rotation.z = x > 0 ? -Math.PI / 2 : Math.PI / 2; cap.position.x = x > 0 ? .75 : -.75; modGroup.add(cap)
    const rim2 = new T.Mesh(new T.TorusGeometry(.58, .03, 8, 24), accentMat)
    rim2.rotation.y = Math.PI / 2; rim2.position.x = x > 0 ? .75 : -.75; modGroup.add(rim2)
    ...
    const eqBox = new T.Mesh(new T.BoxGeometry(.4, .15, .12), darkMat); eqBox.position.set(0, -.62, 0); modGroup.add(eqBox)
  ...
  const buildTruss = (xPos, len) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      const d1 = new T.Mesh(new T.CylinderGeometry(.012, .012, .36, 6), darkMat); d1.position.set(sx, 0, 0); d1.rotation.z = Math.PI / 4; tg.add(d1)
      const d2 = new T.Mesh(new T.CylinderGeometry(.012, .012, .36, 6), darkMat); d2.position.set(sx, 0, 0); d2.rotation.x = Math.PI / 4; tg.add(d2)
    }
    ;[-len / 2, len / 2].forEach(ex => {
      const r = new T.Mesh(new T.TorusGeometry(.18, .015, 6, 12), darkMat); r.rotation.y = Math.PI / 2; r.position.x = ex; tg.add(r)
    })
    return tg
  }
  rootGroup.add(buildTruss(-1.7, .65)); rootGroup.add(buildTruss(1.7, .65))

  /* Solar arrays — yoke arms with two panel wings each */
  ;[-1, 1].forEach(side => {
    const armGroup = new T.Group()
    const yokeBase = new T.Mesh(new T.CylinderGeometry(.13, .13, .35, 12), hullMatDark); yokeBase.position.y = side * .9; armGroup.add(yokeBase)
    const arm = new T.Mesh(new T.CylinderGeometry(.07, .07, 1.7, 10), hullMatDark); arm.position.y = side * 1.95; armGroup.add(arm)
    const gimbal = new T.Mesh(new T.SphereGeometry(.14, 16, 12), goldFoilMat); gimbal.position.y = side * 2.85; armGroup.add(gimbal)
    ;[-1, 1].forEach(wingX => {
      ...
      const bracket = new T.Mesh(new T.BoxGeometry(.18, .25, .25), darkMat); bracket.position.x = wingX > 0 ? -2.3 : 2.3; wing.add(bracket)
      armGroup.add(wing)
    })
  ...
  ;[-1, 1].forEach(side => {
    const rad = new T.Mesh(new T.PlaneGeometry(1.2, .9),
    ...
    const rib = new T.Mesh(new T.BoxGeometry(1.2, .04, .04), darkMat); rib.position.set(side * 1.7, 0, .85); rootGroup.add(rib)
  })

  /* Deep-space communication dish */
  const dishGroup = new T.Group(); dishGroup.position.set(.4, 0, 1.0)
  const dishStem = new T.Mesh(new T.CylinderGeometry(.05, .05, .55, 10), hullMatDark); dishStem.position.y = .27; dishGroup.add(dishStem)
  const dishGimbal = new T.Mesh(new T.SphereGeometry(.08, 12, 10), goldFoilMat); dishGimbal.position.y = .55; dishGroup.add(dishGimbal)
  const dish = new T.Mesh(new T.SphereGeometry(.42, 24, 16, 0, Math.PI * 2, 0, Math.PI / 2.5),
  ...
  const horn = new T.Mesh(new T.ConeGeometry(.06, .18, 10), darkMat); horn.position.set(0, .82, .15); horn.rotation.x = Math.PI; dishGroup.add(horn)
  for (let i = 0; i < 3; i++) {
    const a = (i / 3) * Math.PI * 2
    const strut = new T.Mesh(new T.CylinderGeometry(.006, .006, .25, 6), darkMat)
    strut.position.set(Math.cos(a) * .12, .75, .08 + Math.sin(a) * .12); strut.rotation.x = .4; strut.rotation.z = a; dishGroup.add(strut)
  }
  rootGroup.add(dishGroup)

  /* Secondary antenna */
  const ant2 = new T.Group(); ant2.position.set(-.6, 0, 1.0)
  const ant2Stem = new T.Mesh(new T.CylinderGeometry(.025, .025, .8, 8), darkMat); ant2Stem.position.y = .4; ant2.add(ant2Stem)
  const ant2Tip = new T.Mesh(new T.SphereGeometry(.04, 8, 8), accentMat); ant2Tip.position.y = .82; ant2.add(ant2Tip)
  rootGroup.add(ant2)
  ...
  for (let i = 0; i < 4; i++) {
    ...
    post.position.set(Math.cos(a) * .36, -1.05, .35 + Math.sin(a) * .36); post.rotation.z = -Math.PI / 2; rootGroup.add(post)
  }

  /* RCS thruster quad blocks */
  ;[[3.2, 0, 0], [-3.2, 0, 0], [0, 1.0, 1.0], [0, -1.0, 1.0]].forEach(([x, y, z]) => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      nozzle.position.set(Math.cos(a) * .08, Math.sin(a) * .08, 0); nozzle.rotation.x = Math.PI / 2; quad.add(nozzle)
    }
    rootGroup.add(quad)
  })
  ...
  /* Module accent point lights + halo sprites */
  const moduleColors = [0xc6ff3c, 0x4a78b8, 0x38f5d0, 0xff4d8d]
  const modulePosArr = [[0, .2, .4], [-2.55, 0, 0], [0, .82, .2], [2.55, 0, 0]]
  const moduleFeatureLights = modulePosArr.map((pos, i) => {
    const pl = new T.PointLight(moduleColors[i], 0, 5.5)
    pl.position.set(...pos); rootGroup.add(pl); return pl
  })
  const moduleHalos = modulePosArr.map((pos, i) => {
    const h = new T.Sprite(new T.SpriteMaterial({ map: haloTex, color: moduleColors[i], transparent: true, opacity: 0, blending: T.AdditiveBlending, depthWrite: false }))
    h.scale.set(3.5, 3.5, 1); h.position.set(...pos); rootGroup.add(h); return h
  })

  /* Navigation lights with halo glow */
  const navLights = []
  ;[[3.4, 0, 0, 0xff3050], [-3.4, 0, 0, 0xc6ff3c], [0, 2.95, 1.2, 0xffffff], [0, -2.95, 1.2, 0xffffff], [0, 0, 1.4, 0xff8a3c], [0, 0, -1.4, 0x66ddff]].forEach(([x, y, z, color]) => {
  ...
  const planet = new T.Mesh(
    ...
    new T.MeshStandardMaterial({ color: 0x1a4870, emissive: 0x0a2848, emissiveIntensity: .2, roughness: .7, metalness: .1 })
  )
  planet.position.set(8, -5, -8); rootGroup.add(planet)
  const atmo = new T.Mesh(
    new T.SphereGeometry(2.65, 32, 24),
    new T.ShaderMaterial({
      side: T.BackSide, transparent: true, blending: T.AdditiveBlending, depthWrite: false,
      vertexShader: 'varying vec3 vN;void main(){vN=normalize(normalMatrix*normal);gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}',
      fragmentShader: 'varying vec3 vN;void main(){float d=pow(1.-abs(vN.z),3.);gl_FragColor=vec4(.4,.7,1.,d*.6);}'
    })
  )
  atmo.position.copy(planet.position); rootGroup.add(atmo)

  return { thrusterGlows, moduleHalos, moduleFeatureLights, navLights }
}

/* ── ISSModel – r3f component ────────────────────────────────── */
export const ISSModel = forwardRef(function ISSModel({ thrusterIntensity = 0, activeModule = -1 }, ref) {
  ...
  useFrame(({ clock }) => {
    ...
    const { thrusterGlows, navLights, moduleHalos, moduleFeatureLights } = internalsRef.current
    ...
    navLights.forEach(({ mesh }, i) => {
      const blink = Math.sin(t * (2.1 + i * 0.43) + i * 1.9) > 0.3 ? 1 : 0.04
      mesh.material.opacity = blink
    })

    moduleHalos.forEach((h, i) => {
      const target = i === activeModule ? (0.28 + Math.sin(t * 1.8) * 0.07) : 0
      h.material.opacity += (target - h.material.opacity) * 0.06
    })
    moduleFeatureLights.forEach((l, i) => {
      const target = i === activeModule ? (1.5 + Math.sin(t * 2.2) * 0.4) : 0
      l.intensity += (target - l.intensity) * 0.06
    })
  })
