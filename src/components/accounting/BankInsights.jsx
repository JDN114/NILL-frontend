...
// Updated: migrated from Tailwind to ac-* design system
    ...
    <div>
      <div className="ac-kpi-grid" style={{ marginBottom:16 }}>
        {status && (
          <div className="ac-kpi">
            ...
            <div className={`ac-kpi-value ${(status.balance||0) >= 0 ? "green" : "pink"}`}>
              {fmtEur(status.balance)}
            </div>
            <div className="ac-kpi-delta">{status.bank_name || ""} • {status.iban || ""}</div>
          </div>
        )}
        <div className="ac-kpi">
      ...
      <div className="ac-card" style={{ padding:0 }}>
        <table className="ac-table">
          ...
          <tbody>
            {filtered.length === 0 && (
              <tr><td colSpan={5} className="ac-empty">Keine Transaktionen gefunden.</td></tr>
            )}
