...
// Financial reports: Bilanz, GuV, EÜR, BWA, Saldenliste
  ...
  if (!data) return (
    <div className="ac-card">
      <div className="ac-empty">
        <button className="ac-btn ac-btn-primary" onClick={load}>Saldenliste laden</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="ac-card" style={{ padding:0 }}>
      <div style={{ padding:"16px 20px" }}>
        <span className="ac-section-title">Saldenliste {von} – {bis}</span>
      </div>
      <table className="ac-table" style={{ fontSize:".82rem" }}>
          ...
          {(data.konten || []).map(k => (
            <tr key={k.kontonummer}>
              ...
              <td className="ac-mono" style={{textAlign:"right",
                color: (k.saldo||0) < 0 ? "var(--a3)" : "var(--accent)"}}>
                {fmtEur(k.saldo)}
              </td>
            </tr>
          ))}
        ...
        <tfoot>
          <tr style={{ borderTop:"1px solid rgba(239,237,231,.15)" }}>
            ...
            <td className="ac-mono" style={{textAlign:"right", padding:"12px 14px", fontWeight:600}}>
              {fmtEur(data.summe_soll)}
            </td>
            <td className="ac-mono" style={{textAlign:"right", padding:"12px 14px", fontWeight:600}}>
              {fmtEur(data.summe_haben)}
            </td>
            <td className="ac-mono" style={{textAlign:"right", padding:"12px 14px", fontWeight:600,
              color: Math.abs(data.summe_soll - data.summe_haben) < 0.01 ? "var(--accent)" : "var(--a3)"}}>
...
function Bilanz({ stichtag }) {
  ...
  if (!data) return (
    <div className="ac-card">
      <div className="ac-empty">
        <button className="ac-btn ac-btn-primary" onClick={load}>Bilanz laden (Stichtag: {stichtag})</button>
      </div>
    </div>
  );
  ...
  const renderSection = (title, items, color) => (
    <div style={{ marginBottom:16 }}>
      <div style={{ fontFamily:"Fraunces,serif", fontWeight:600, fontSize:"1rem",
        borderBottom:"1px solid var(--border)", paddingBottom:8, marginBottom:8 }}>
        {title}
      </div>
      {(items || []).map((item, i) => (
        <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"4px 0",
          paddingLeft: item.indent ? (item.indent * 16)+"px" : "0" }}>
          <span style={{ fontSize:".85rem", color: item.bold ? "var(--ink)" : "var(--ink2)" }}>
            {item.bezeichnung}
          </span>
          <span className="ac-mono" style={{ color, fontWeight: item.bold ? 700 : 400, fontSize:".85rem" }}>
            {fmtEur(item.betrag)}
          </span>
        </div>
      ))}
  ...
  return (
    <div className="ac-grid-2">
      <div className="ac-card">
        ...
        {renderSection("A. Anlagevermögen", data.aktiva?.anlage, "var(--accent)")}
        {renderSection("B. Umlaufvermögen", data.aktiva?.umlauf, "var(--accent)")}
        <div style={{ borderTop:"2px solid var(--accent)", paddingTop:12, display:"flex", justifyContent:"space-between" }}>
          ...
          <span className="ac-mono" style={{ color:"var(--accent)", fontWeight:700, fontSize:"1.1rem" }}>
            {fmtEur(data.aktiva?.summe)}
          </span>
        </div>
      </div>
      <div className="ac-card">
        ...
        {renderSection("B. Rückstellungen", data.passiva?.rueckstellungen, "var(--a2)")}
        ...
        <div style={{ borderTop:"2px solid var(--a2)", paddingTop:12, display:"flex", justifyContent:"space-between" }}>
          ...
          <span className="ac-mono" style={{ color:"var(--a2)", fontWeight:700, fontSize:"1.1rem" }}>
            {fmtEur(data.passiva?.summe)}
          </span>
        </div>
      </div>
    </div>
...
function GuV({ von, bis }) {
  ...
  if (!data) return (
    <div className="ac-card">
      <div className="ac-empty">
        <button className="ac-btn ac-btn-primary" onClick={load}>GuV laden</button>
      </div>
    </div>
  );
    ...
    <div className="ac-card">
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:20 }}>
        <div>
          ...
          {[
            ["1. Umsatzerlöse",                    data.umsatzerloese],
            ["2. Bestandsveränderungen",            data.bestandsveraenderungen],
            ["3. Sonstige betr. Erträge",           data.sonstige_ertraege],
            ["Gesamtleistung",                      data.gesamtleistung, true],
            ...
            <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"5px 0",
              ...
              <span className="ac-mono" style={{ color:"var(--accent)", fontWeight: bold ? 700 : 400, fontSize:".85rem" }}>
                {fmtEur(val)}
              </span>
            </div>
          ))}
        ...
        <div>
          ...
          {[
            ["4. Materialaufwand",                  data.materialaufwand],
            ["6. Personalaufwand",                  data.personalaufwand],
            ["7. Abschreibungen",                   data.abschreibungen],
            ["8. Sonstige betr. Aufwendungen",      data.sonstige_aufwendungen],
            ["Gesamtkosten",                        data.gesamtkosten, true],
          ].map(([label, val, bold], i) => (
            <div key={i} style={{ display:"flex", justifyContent:"space-between", padding:"5px 0",
              borderTop: bold ? "1px solid var(--border)" : "none" }}>
              <span style={{ fontSize:".85rem", color: bold ? "var(--ink)" : "var(--ink2)" }}>{label}</span>
              <span className="ac-mono" style={{ color:"var(--a3)", fontWeight: bold ? 700 : 400, fontSize:".85rem" }}>
                {fmtEur(val)}
              </span>
            </div>
          ))}
      ...
      <div style={{ borderTop:"2px solid var(--border)", paddingTop:16, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <span style={{ fontFamily:"Fraunces,serif", fontWeight:700, fontSize:"1.1rem" }}>
          {gewinn >= 0 ? "Jahresüberschuss" : "Jahresfehlbetrag"}
        </span>
        <span className="ac-mono" style={{ fontSize:"1.5rem", fontWeight:700,
          color: gewinn >= 0 ? "var(--accent)" : "var(--a3)" }}>
          {fmtEur(Math.abs(gewinn))}
        </span>
      </div>
    </div>
  );
...
function EueR({ jahr }) {
  ...
  if (!data) return (
    <div className="ac-card">
      <div className="ac-empty">
        <button className="ac-btn ac-btn-primary" onClick={load}>EÜR {jahr} laden</button>
      </div>
    </div>
  );
...
function BWA({ von, bis }) {
  ...
  if (!data) return (
    <div className="ac-card">
      <div className="ac-empty">
        <button className="ac-btn ac-btn-primary" onClick={load}>BWA laden</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="ac-card">
      ...
      <table className="ac-table" style={{ fontSize:".85rem" }}>
        <thead>
          <tr><th>Position</th><th style={{textAlign:"right"}}>Betrag</th><th style={{textAlign:"right"}}>%</th></tr>
        </thead>
        <tbody>
          {(data.positionen || []).map((p, i) => (
            <tr key={i} style={{ fontWeight: p.summe ? 600 : 400 }}>
              ...
              <td className="ac-mono" style={{ textAlign:"right",
                color: p.betrag < 0 ? "var(--a3)" : p.summe ? "var(--accent)" : "var(--ink)" }}>
                {fmtEur(p.betrag)}
              </td>
              <td className="ac-mono" style={{ textAlign:"right", color:"var(--ink2)" }}>
                {p.prozent !== undefined ? `${Number(p.prozent).toFixed(1)} %` : "—"}
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    <div>
      <div style={{ display:"flex", gap:4, flexWrap:"wrap", marginBottom:16 }}>
        {BERICHT_TABS.map(t => (
          <button key={t.id}
            className={`ac-btn ${bericht===t.id ? "ac-btn-primary" : "ac-btn-ghost"}`}
            onClick={() => setBericht(t.id)}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {bericht==="bilanz" && (
        <div className="ac-form-row" style={{ marginBottom:16 }}>
          <div className="ac-form-col" style={{ maxWidth:220 }}>
            <label className="ac-label">Bilanzstichtag</label>
            <input className="ac-input" type="date" value={stichtag} onChange={e => setStichtag(e.target.value)} />
          </div>
        </div>
      )}
