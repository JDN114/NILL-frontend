...
// Double-entry journal: list, create, storno
...
function NeuBuchungModal({ konten, perioden, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    buchungstext: "",
    beleg_nummer: "",
    periode_id: perioden[0]?.id || "",
    buchungstyp: "standard",
  });
  ...
  const addZeile = () =>
    setZeilen(z => [...z, { konto_id: "", soll: "", haben: "", ust_kennzeichen: "" }]);

  const removeZeile = (i) =>
    setZeilen(z => z.filter((_, idx) => idx !== i));

  const setZeile = (i, key, val) =>
    setZeilen(z => z.map((row, idx) => idx === i ? { ...row, [key]: val } : row));
        ...
        <div style={{ marginBottom: 12 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
            ...
            <span style={{ fontSize:".8rem", color: balanced ? "var(--accent)" : "var(--a3)" }}>
              Soll: {fmt(sollSumme)} | Haben: {fmt(habenSumme)} {balanced ? "✓" : "✗"}
            </span>
          </div>
          ...
          {zeilen.map((z, i) => (
            <div key={i} className="ac-form-row" style={{ marginBottom: 8 }}>
              ...
              <button className="ac-btn ac-btn-danger ac-btn-sm" onClick={() => removeZeile(i)}
                disabled={zeilen.length <= 2}>×</button>
            </div>
          ))}
...
export default function BuchungenTab() {
  ...
  const storno = async (id) => {
    if (!window.confirm("Buchungssatz stornieren? Dies erstellt einen Gegenbuchungssatz.")) return;
    try {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        ...
        <button className="ac-btn ac-btn-primary" onClick={() => setShowModal(true)}>
          + Buchungssatz
        </button>
      </div>
                  ...
                  <td>
                    {!b.festgeschrieben && !b.ist_storno && (
                      <button className="ac-btn ac-btn-danger ac-btn-sm"
                        onClick={e => { e.stopPropagation(); storno(b.id); }}>
                        Storno
                      </button>
                    )}
                  </td>
